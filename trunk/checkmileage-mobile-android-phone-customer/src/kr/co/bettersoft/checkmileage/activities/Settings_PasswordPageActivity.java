...
public class Settings_PasswordPageActivity extends PreferenceActivity implements OnSharedPreferenceChangeListener{
	...
	/**
	 * onResume
	 *  리쥼시 프리퍼런스의 비번 값을 통해 비번 사용 체크박스를 활성화/비활성화 시킨다. (비번이 없으면 비번 사용 여부 선택 불가)
	 *
	 * @param
	 * @param
	 * @return
	 ...
	 *  프리퍼런스(설정)에서 설정 메뉴 클릭시 발생하는 이벤트를처리한다.
	 *
	 ...
	 *  프리퍼런스 값이 바뀌었을때에 동작한다. 공유 프리퍼런스와 디폴트 프리퍼런스의 동기화에 사용된다.
	 *
